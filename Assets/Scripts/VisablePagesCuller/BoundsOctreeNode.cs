...
namespace SB
{
	...
	{
		bool IsVisible(ref Bounds bounds);
	}
	...
	public class BoundsOctreeNode<T>
	{
		...
		public static IExtendCheck extendCheckInterface { private get; set; } = null;
		public void GetWithinFrustum(Plane[] planes, T[] result, ref uint count)
		{
			// Is the input node inside the frustum?
			if (!GeometryUtility.TestPlanesAABB(planes, bounds))
				return;
			...
			for (int i = 0; i < objects.Count; i++)
			{
				if (GeometryUtility.TestPlanesAABB(planes, objects[i].Bounds))
				{
					testBounds = objects[i].Bounds;

					if ((extendCheckInterface == null) ||
						extendCheckInterface.IsVisible(ref testBounds))
					{
						result[count] = objects[i].Obj;
						++count;
					}
				}
			}
			...
			if (children != null)
			{
				for (int i = 0; i < 8; i++)
				{
					children[i].GetWithinFrustum(planes, result, ref count);
				}
			}
		}
		...
		public void DrawAllObjects()
		{
			...
			Gizmos.color = new Color(1.0f, 1.0f - tintVal, tintVal, 0.25f);
