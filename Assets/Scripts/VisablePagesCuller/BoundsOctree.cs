...
namespace SB
{
	...
	{
		public static Plane[] GetFrustumPlanes() { return ms_tempPlanes; }
		...
		public void GetWithinFrustum(Camera cam, List<T> destBounds)
		{
			if (destBounds == null)
				return;
			GeometryUtility.CalculateFrustumPlanes(cam,
				ms_tempPlanes);
			rootNode.GetWithinFrustum(ms_tempPlanes, destBounds);
		}

		public void GetWithinFrustum(Camera cam, T[] destBounds, ref uint count)
		{
			if (destBounds == null)
				return;
			GeometryUtility.CalculateFrustumPlanes(cam,
				ms_tempPlanes);
			rootNode.GetWithinFrustum(ms_tempPlanes, destBounds, ref count);
		}

		private static Plane[] ms_tempPlanes = new Plane[6];
