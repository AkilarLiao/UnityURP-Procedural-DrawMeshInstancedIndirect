...
namespace SB
{
    public class CollisionManager : Singleton<CollisionManager>
    {
        ...
        // event is keyword, give delegate's member
        // can't use = or invok, may sure be use by external, cause bug.
        public delegate void TouchEventHandler(TouchInformation touchInfo,
        ...
        private void SetupPerTouchInformation(ref Touch theTouch,
            ref TouchInformation touchInformation)
        {
        ...
        private void SetupTouchInformation()
        {
            ...
            if (Input.GetMouseButton(0))
            {
                ...
                {
                    m_touchInformation.m_touchDelta.x -= 
                        Input.GetAxis("Mouse X");
                    m_touchInformation.m_touchDelta.y -=
                        Input.GetAxis("Mouse Y");
                    if ((Mathf.Abs(
                        m_touchInformation.m_touchDelta.x) > 0.0f) ||
                        (Mathf.Abs(m_touchInformation.m_touchDelta.y)
                        > 0.0f))
            ...
            if (Input.touchCount >= 2)
            {
                ...
                SetupPerTouchInformation(ref theTouch,
                ref m_touchInformation1);
            }
#endif
        }
            ...
            if (m_touchInformation.m_type != TOUCH_EVENT_TYPE.NONE)
            {
                ...
                if (injectGUI == false)
                {
                    if (Camera.main)
                    {
                        Ray testRay = Camera.main.ScreenPointToRay(
                            new Vector3(m_touchInformation.m_touchPosition.x,
                            m_touchInformation.m_touchPosition.y,
                            0.0f));
                        ...
                        for (int index = 0; index<theHits.Length; ++index)
                        {
                            ...
                            {
                                compareDistance = theHits[index].distance;
                                hitGameObject = theHits[index].collider.
                                    gameObject;
                            }
                        }
                    }
                ...
                switch (m_touchInformation.m_type)
                {
                    case TOUCH_EVENT_TYPE.PRESS:
                    {
                        ...
                        {
                            m_pressEventHandler(m_touchInformation,
                                m_touchInformation1, injectGUI,
                                hitGameObject);
                        }
                    }
                    break;
                    case TOUCH_EVENT_TYPE.RELEASE:
                    {
                        ...
                        {
                            m_releaseEventHandler(m_touchInformation,
                                m_touchInformation1, injectGUI,
                                hitGameObject);
                        }
                    }
                    ...
                    case TOUCH_EVENT_TYPE.MOVE:
                    {
                        ...
                        {
                            m_moveEventHandler(m_touchInformation,
                                m_touchInformation1, injectGUI,
                                hitGameObject);
                        }
                    }
