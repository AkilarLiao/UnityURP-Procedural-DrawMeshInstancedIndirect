...
namespace SB
{
    public class CameraControler : MonoBehaviour
    {
        ...
        public float AccumulateMoveRatio { 
            set { m_accumulateMoveRatio = value; } }
        ...
        private void OnEnable()
        {
            ...
            {
                CollisionManager.Instance.m_pressEventHandler += 
                    ProcessTouchPress;
                CollisionManager.Instance.m_moveEventHandler += 
                    ProcessTouchMove;
                CollisionManager.Instance.m_releaseEventHandler +=
                    ProcessTouchRelease;
            }
        ...
        void AppendExtendStringCB(ref string text)
        {
            if (m_ShowPosition)
            {
        ...
        private void OnDisable()
        {
            ...
            if (!CollisionManager.IsApplicationIsQuitting() &&
        ...
        private void Update()
        {
            ...
            float fDestRatio = Mathf.Min(mc_fDeltaMoveScaleRatio *
                Time.deltaTime, 1.0f);
                ...
                (Mathf.Abs(DestVelocity.y) > 0.0f))
            {
                if (!m_isRoomIn)
                {
                    ...
                    {
                        Debug.Log(string.Format("Happen Huge Distance: {0}",
                            FixVelocity.magnitude));
                    }
                }
            }
        ...
        {
            offestRange = 0.0f;
            float roomInDistance = m_roomInDistance;
            ...
            offestRange = (m_oldTouchPosition1 - m_oldTouchPosition2).
                sqrMagnitude < (touchPosition1 - touchPosition2).
                sqrMagnitude ? -m_roomInDistance : m_roomInDistance;
        ...
        private void ProcessTouchPress(
            CollisionManager.TouchInformation touchInfo,
            CollisionManager.TouchInformation touchInfo1, bool injectGUI,
            ...
            GameObject hitGameObject)
        {
            ...
            {
                Vector2 DeletaRange = m_touchPressPosition -
                    touchInfo.m_touchPosition;
                if ((m_pressTime < mc_fMaxPressTime) &&
                    (DeletaRange.sqrMagnitude >= mc_fMoveCheckRange *
                    mc_fMoveCheckRange))
                {   
                    m_deltaMovement -= (DeletaRange / m_pressTime) *
                        mc_fReleaseMoveTime * m_accumulateMoveRatio;
                }
            }
            m_pressed = false;
            m_pressTime = 0.0f;
        ...
        private void GetMoveDirection(out Vector3 faceDirection,
            out Vector3 rightDirection)
        {
            ...
            rightDirection = Quaternion.AngleAxis(90.0f, Vector3.up) * 
                faceDirection;
        }

        private bool IsEnlarge(Vector2 oldP1, Vector2 oldP2, Vector2 newP1,
            Vector2 newP2)
        {
        ...
        private void ProcessPressMovement(
            CollisionManager.TouchInformation touchInfo)
        {
            if (!m_isRoomIn)
            {
                ...
                DestVelocity += touchInfo.m_touchDelta.y * m_speed * 
                    FaceDirection;
                DestVelocity += touchInfo.m_touchDelta.x * m_speed * 
                    RightDirection;                
        ...
        [SerializeField]
        private SECOND_TOUCH_MODE m_secondTouchMode = 
            SECOND_TOUCH_MODE.ROTATE;

        [SerializeField]
        private float m_roomInMinHeight = 45.0f;

        [SerializeField]
        private float m_roomInMaxHeight = 3072.0f;
        ...
        private bool[] m_theMoveStates =
            new bool[(int)MoveState.MoveState_Max];
